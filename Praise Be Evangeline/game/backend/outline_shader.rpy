################################################################################
##
## Outline Shader for Ren'Py v1.2
##
################################################################################
...
################################################################################
## NEW IN v1.2: Web build update:
##
## IF YOU ARE USING OUTLINES LARGER THAN 50px, find this line in the shader:
##      const float max_width = 50.0;
## Change it to the largest outline size you use. Performance may be improved
## by changing this to the maximum outline width you use if it's smaller than
## 50 (particularly for web builds).
##
## IF YOUR RESOLUTION IS LARGER THAN 1920 IN ONE DIMENSION,
## find this line in the shader:
##      const float min_px_size = 1.0/1920.0;
## Change 1920 to the maximum resolution you need. Performance may be improved
## by setting 1920 to the smallest resolution you use, or further by setting
## min_px_size to the smallest value of width/num_passes used for all uses
## of the shader.
##
## IF YOU ARE USING LESS THAN 45.0 FOR SMOOTHING, you can adjust this line:
##      const float max_angles = 45.0*8.0;
## Change 45.0 to the highest number of smoothing you use across all uses of
## the shader. This may result in performance improvements on web.
################################################################################
...
init -50 python:
    ...
    """, fragment_300="""
        ...
        // Put a hard limit on the number of iterations. Used so the
        // shader doesn't complain on web builds.
        // See: https://stackoverflow.com/questions/38986208/webgl-loop-index-cannot-be-compared-with-non-constant-expression
        const float max_width = 50.0;
        const float max_angles = 45.0*8.0;
        const float min_px_size = 1.0/1920.0;

        const float upper_pass_limit = max_width / min_px_size;
        float decrease_num = max(
            max(u_width/u_num_passes,
            min(pixel_size.x, pixel_size.y)), min_px_size);
        float start_num = min(u_width, max_width);
        float num = start_num;
        ...
        for (float fake_num=0.0; fake_num<upper_pass_limit; fake_num++) {
            // Mimicking this loop; the numbers are constants for web builds.
            // for (float num=start_num; num>=0.0; num-=decrease_num)
            num = start_num - fake_num * decrease_num;
            if (num < 0.0) break;

            // This loop runs for the number of angles the shader is
            // considering. The more angles, the rounder and smoother the
            // final outline.
            for (float i=0.0; i < max_angles; i++) {
                if (i >= angle_smooth) break;
